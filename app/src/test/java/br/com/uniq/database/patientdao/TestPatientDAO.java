package br.com.uniq.database.patientdao;
...
public class TestPatientDAO {
    ...
    void signUp_testInsertSomePatient() throws Exception {
        ...
        assertEquals(rsPatient.toString(), patient.toString());
        Exception exception = assertThrows(Exception.class, () -> PatientDAO.getInstance().signUp(patient));
    ...
    void signUp_testInvalidValues() {
        Exception exception = assertThrows(Exception.class, () -> PatientDAO.getInstance().signUp(null));
        assertEquals("Paciente não fornecido", exception.getMessage());

        Patient patient1 = new Patient("João", "123", 15, "123");
        Exception exception1 = assertThrows(Exception.class, () -> PatientDAO.getInstance().signUp(patient1));
        assertEquals("CPF inválido!", exception1.getMessage());

        Patient patient2 = new Patient("Cláudio", "12345678901", 121, "123");
        Exception exception2 = assertThrows(Exception.class, () -> PatientDAO.getInstance().signUp(patient2));
